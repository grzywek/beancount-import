...
- positions_YYYY-MM-DD.json: Optional dated positions snapshot for Balance directives
- account_summary_YYYY-MM-DD.json: Optional dated cash snapshot for Balance directives
...
class PositionBalanceSnapshot:
    """Dated positions snapshot used only for balance assertions."""
    date: datetime.date
    source_file: str
...
class CashBalanceSnapshot:
    """Dated account summary snapshot used only for cash balance assertions."""
    date: datetime.date
    source_file: str
    summary: ApiAccountSummary
...
class Trading212Source(DescriptionBasedSource):
    ...
    def __init__(
        ...
        **kwargs,  # Accept extra config params (e.g., 'description')
        ...
        self._position_balance_snapshots: Optional[List[PositionBalanceSnapshot]] = None
        self._cash_balance_snapshots: Optional[List[CashBalanceSnapshot]] = None
    ...
    def _load_json_path(self, path: str) -> Optional[dict]:
        """Load a JSON file by absolute path."""
        ...
        try:
            ...
            print(f"[Trading212] Error loading {os.path.basename(path)}: {e}", flush=True)
            return None

    def _account_summary_from_dict(self, data: dict) -> ApiAccountSummary:
        """Parse an account summary JSON object."""
    ...
    def _get_dated_json_snapshots(self, prefix: str) -> List[Tuple[datetime.date, str, dict]]:
        """Load JSON snapshots named PREFIX_YYYY-MM-DD.json."""
        pattern = os.path.join(self.data_directory, f"{prefix}_[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9].json")
        snapshots = []
        for path in sorted(glob.glob(pattern)):
            basename = os.path.basename(path)
            date_part = basename[len(prefix) + 1:-len(".json")]
            try:
                snapshot_date = datetime.date.fromisoformat(date_part)
            except ValueError:
                print(f"[Trading212] Warning: Ignoring dated snapshot with invalid date: {basename}", flush=True)
                continue
            data = self._load_json_path(path)
            if data is not None:
                snapshots.append((snapshot_date, path, data))
        return snapshots

    def _load_position_balance_snapshots(self) -> List[PositionBalanceSnapshot]:
        """Load positions_YYYY-MM-DD.json files for balance assertions."""
        snapshots = []
        for snapshot_date, path, data in self._get_dated_json_snapshots("positions"):
            if "items" not in data:
                print(f"[Trading212] Warning: Ignoring {os.path.basename(path)} without items", flush=True)
                continue
            snapshots.append(PositionBalanceSnapshot(
                date=snapshot_date,
                source_file=path,
                positions=[self._dict_to_position(item) for item in data["items"]],
            ))
        return snapshots

    def _load_cash_balance_snapshots(self) -> List[CashBalanceSnapshot]:
        """Load account_summary_YYYY-MM-DD.json files for cash balance assertions."""
        return [
            CashBalanceSnapshot(
                date=snapshot_date,
                source_file=path,
                summary=self._account_summary_from_dict(data),
            )
            for snapshot_date, path, data in self._get_dated_json_snapshots("account_summary")
        ]
    ...
    def _load_all_data(self) -> None:
        ...
        if self._account_summary is None:
            ...
            if data:
                self._account_summary = self._account_summary_from_dict(data)
                # Parse fetched_at date
                if "fetched_at" in data:
                    fetched_at = datetime.datetime.fromisoformat(data["fetched_at"])
        ...
        # Load dated balance snapshots. These are opt-in: regular positions.json
        # and account_summary.json are not used to create Balance directives.
        if self._position_balance_snapshots is None:
            print("\n[Trading212] === POSITION BALANCE SNAPSHOTS ===", flush=True)
            self._position_balance_snapshots = self._load_position_balance_snapshots()
            print(f"[Trading212] ✓ {len(self._position_balance_snapshots)} dated position snapshot(s) loaded", flush=True)

        if self._cash_balance_snapshots is None:
            print("\n[Trading212] === CASH BALANCE SNAPSHOTS ===", flush=True)
            self._cash_balance_snapshots = self._load_cash_balance_snapshots()
            print(f"[Trading212] ✓ {len(self._cash_balance_snapshots)} dated cash snapshot(s) loaded", flush=True)
        ...
        print(f"  Position balance snapshots: {len(self._position_balance_snapshots or [])}", flush=True)
        print(f"  Cash balance snapshots: {len(self._cash_balance_snapshots or [])}", flush=True)
    ...
    def _build_symbol_registry(self) -> None:
        ...
        # Dated position snapshots can be the only place a balance commodity appears.
        if self._position_balance_snapshots:
            for snapshot in self._position_balance_snapshots:
    ...
    def _make_balance_assertion(
        ...
        if source_file:
            meta[SOURCE_DOC_KEY] = os.path.basename(source_file)
    ...
    def _make_cash_balance_assertion(
        self,
        summary: ApiAccountSummary,
        date: datetime.date,
        source_file: Optional[str] = None,
    ...
    def prepare(self, journal: JournalEditor, results: SourceResults) -> None:
        ...
        # Generate balance assertions only from explicit dated snapshots.
        # Regular positions.json and account_summary.json remain current-state
        # inputs for reconciliation/control reports, but do not create balances.
        for snapshot in self._position_balance_snapshots or []:
            for position in snapshot.positions:
                symbol_account = self._get_symbol_account(position.ticker, position.isin)
                account_set.add(symbol_account)

                balance = self._make_balance_assertion(position, snapshot.date, snapshot.source_file)
                results.add_pending_entry(ImportResult(
                    date=snapshot.date,
                    entries=[balance],
                    info={
                        ...
                        "document": os.path.basename(snapshot.source_file),
                    },
                ))

        for snapshot in self._cash_balance_snapshots or []:
            cash_balance = self._make_cash_balance_assertion(
                snapshot.summary,
                snapshot.date,
        ...
        balance_snapshot_docs = {
            snapshot.source_file: snapshot.date
            for snapshot in (self._position_balance_snapshots or [])
        }
        balance_snapshot_docs.update({
            snapshot.source_file: snapshot.date
            for snapshot in (self._cash_balance_snapshots or [])
        })
        for json_path, snapshot_date in sorted(balance_snapshot_docs.items()):
            doc_basename = os.path.basename(json_path)
            results.add_pending_entry(ImportResult(
                date=snapshot_date,
                entries=[
                    Document(
